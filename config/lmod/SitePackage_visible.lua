function visible_hook(t)
	...
	-- only test visibility for modules in our CVMFS repo
	local prefix = os.getenv("CCR_CVMFS_REPO")
	...
	local restricted_available = os.getenv("CCR_RESTRICTED") or "false"
	...
	if moduleName == "cuda" then
		...
		local driver_version = os.getenv("CCR_CUDA_DRIVER_VERSION") or "0"
