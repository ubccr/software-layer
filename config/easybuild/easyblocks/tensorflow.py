...
class EB_TensorFlow(PythonPackage):
    ...
    def get_system_libs(self):
        ...
        libpaths.append('/opt/software/nvidia/lib64')
        for cuda_lib in ['CUDA', 'cuDNN', 'NCCL']:
            cuda_lib_root = get_software_root(cuda_lib)
            if cuda_lib_root:
                incpath = os.path.join(cuda_lib_root, 'include')
                if os.path.exists(incpath):
                    cpaths.append(incpath)
                libpath = get_software_libdir(cuda_lib)
                if libpath:
                    libpaths.append(os.path.join(cuda_lib_root, libpath))
    ...
    def configure_step(self):
        ...
        if build_option('rpath'):
            if self.toolchain.options.get('rpath', True):
                orig_ar = which('ar')
                if not os.path.exists(os.path.join(self.binutils_bin_path, 'ar')):
                    os.symlink(orig_ar, os.path.join(self.binutils_bin_path, 'ar'))
    ...
    def test_step(self):
        ...
        for device in devices:
            ...
            if device == CPU_DEVICE:
                ...
                # XXX this was required to get conv3d_backprop_filter_v2_grad_test_cpu to pass
                # for TensorFlow-2.9.1
                current_test_opts.append('--test_env=TF_ENABLE_ONEDNN_OPTS=0')
            else:
