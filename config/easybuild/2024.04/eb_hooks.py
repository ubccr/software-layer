...
#    openssl_ver_replace(ec, eprefix)
    modify_dependencies(ec)
...
def cuda_config_opts(ec, prefix):
    ...
    if ec.name == 'CUDA' or ec.name == 'cuDNN':
        ec.update('builddependencies', [('GCCcore', '12.3.0')])
...
def rust_config_opts(ec, prefix):
    """Custom config options for rust."""
    if ec.name != 'Rust':
        raise EasyBuildError("rust-specific hook triggered for non-perl easyconfig?!")

    print_msg(f"Set path to openssl in compat layer for Rust {ec.version}..")
    setvar("EBROOTOPENSSL", f"{prefix}/usr")
...
def pillow_preconfig(ec, *args, **kwargs):
    ...
    eprefix = get_ccr_envvar('EPREFIX')
    setvar("LDFLAGS", f'-L{eprefix}/usr/lib64')
...
def openssl_ver_replace(ec, eprefix):
    for index in range(len(ec['dependencies'])):
        dep = ec['dependencies'][index]
        if isinstance(dep, (list,tuple)) and (dep[0] == "OpenSSL" and dep[1] == '1.1'):
            print_msg("NOTE:OpenSSL dependency version has been modified from OpenSSL/1.1 --> OpenSSL/3")
            if isinstance(dep, list):
                ec['dependencies'][index] = ["OpenSSL", "3"]
            else:
                ec['dependencies'][index] = ("OpenSSL", "3")
    ...
    for index in range(len(ec['hiddendependencies'])):
        ...
        if isinstance(dep, (list,tuple)) and (dep[0] == "OpenSSL" and dep[1] == '1.1'):
            print_msg("NOTE:OpenSSL hiddendependency version has been modified from OpenSSL/1.1 --> OpenSSL/3 ")
            if isinstance(dep, list):
                ec['hiddendependencies'][index] = ["OpenSSL", "3"]
            else:
                ec['hiddendependencies'][index] = ("OpenSSL", "3")
    for index in range(len(ec['builddependencies'])):
        dep = ec['builddependencies'][index]
        if isinstance(dep, (list,tuple)) and (dep[0] == "OpenSSL" and dep[1] == '1.1'):
            print_msg("NOTE:OpenSSL builddependency version has been modified from OpenSSL/1.1 --> OpenSSL/3")
            if isinstance(dep, list):
                ec['builddependencies'][index] = ["OpenSSL", "3"]
            else:
                ec['builddependencies'][index] = ("OpenSSL", "3")

def modify_dependencies(ec):
    modify_dep(ec, 'OpenSSL', '3', '1.1')
    modify_dep(ec, 'Catch2', '2.13.9', from_version='2.13.9', toolchain=('GCCcore', '13.2.0'))

def modify_dep(ec, name, to_version, from_version=None, suffix='', toolchain=None):
    for key in ['dependencies', 'hiddendependencies', 'builddependencies']:
        for index in range(len(ec[key])):
            dep = ec[key][index]
            if isinstance(dep, (list,tuple)) and dep[0] == name:
                if from_version != None and dep[1] != from_version:
                    continue

                print_msg(f"NOTE:{name} {key} version has been modified from {name}/{dep[1]} --> {name}/{to_version}")
                if toolchain != None and isinstance(toolchain, tuple):
                    ec[key][index] = (name, to_version, suffix, toolchain)
                else:
                    ec[key][index] = (name, to_version)
