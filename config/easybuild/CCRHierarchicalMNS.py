...
class CCRHierarchicalMNS(ModuleNamingScheme):
    ...
    def is_short_modname_for(self, short_modname, name):
        ...
        return bool(modname_regex.match(short_modname.lower()))
    ...
    def det_modpath_extensions(self, ec):
        ...
        if is_compiler or non_system_cuda:
            ...
            # XXX for compilers this has to be dynamically built. So it's set in eb_hooks via modluafooter
        ...
        if os.getenv('CCR_ARCH') is None:
            raise EasyBuildError("Need to set architecture for MODULEPATH extension in $CCR_ARCH")
        if ec['name'] != 'CUDAcore':
            for i, path in enumerate(paths):
                paths[i] = os.path.join(os.getenv('CCR_ARCH'), path)
        return paths
